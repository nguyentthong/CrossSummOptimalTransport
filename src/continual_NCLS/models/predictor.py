...
from src.continual_NCLS.others.utils import rouge_results_to_str, test_rouge, tile
from src.continual_NCLS.translate.beam import GNMTGlobalScorer
import time
import numpy as np
...
class Translator(object):
    ...
    def from_batch(self, translation_batch):
        ...
        for b in range(batch_size):
            ...
            if self.args.predict_chinese:
                ...
                pred_tokens = self.vocab.tokenize(pred_str)
                tgt_tokens = self.vocab.tokenize(tgt_str)
    ...
    def translate(self,
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for i in range(len(batch_data['dec_hidden_states'])): dec_hidden_states.append(batch_data['dec_hidden_states'][i][0])
        ...
        if (step != -1):
            torch.save(dec_hidden_states, self.args.save_dec_hidden_states_path)
    ...
    def _fast_translate_batch(self,
        ...
        results['dec_hidden_states'] = [[] for _ in range(batch_size)]
        dec_hidden_states = []
        ...
        for step in range(max_length):
            ...
            dec_hidden_states.append(dec_out.transpose(0,1).squeeze(0))
            # Generator forward.
            ...
            alive_seq = torch.cat([alive_seq.index_select(0, torch.tensor(select_indices,dtype=torch.int64).to('cuda')), topk_ids.view(-1, 1)], -1)
            ...
            language_segs = language_segs.index_select(0, torch.tensor(select_indices,dtype=torch.int64).to('cuda')).to('cuda')

            last_segs = language_segs[:, -1]
            tmp_seg = last_segs.masked_fill(is_languaged.flatten(), 1)
            ...
            if is_finished.any():
                dec_h = torch.stack(dec_hidden_states, 1)
                dec_h = dec_h.view(-1, beam_size, dec_h.size(-2), dec_h.size(-1))
                ...
                for i in range(is_finished.size(0)):
                    ...
                    if end_condition[i]:
                        ...
                        best_hyp_arg = max(range(len(hypotheses[b])), key=lambda i: hypotheses[b][i][0])

                        score, pred = best_hyp[0]
                        results["scores"][b].append(score)
                        results["predictions"][b].append(pred)

                        results['dec_hidden_states'][b].append(dec_h[i][best_hyp_arg%beam_size])
                ...
                dec_hidden_states = list(torch.unbind(dec_h.index_select(0, non_finished).view(dec_h.size(-2), -1, dec_h.size(-1)),0))

            # Reorder states.
            select_indices = batch_index.view(-1)
            src_features = src_features.index_select(0, torch.tensor(select_indices, dtype=torch.int64).to('cuda'))
            dec_states.map_batch_fn(
                lambda state, dim: state.index_select(dim, torch.tensor(select_indices, dtype=torch.int64).to('cuda')))
